...
CYCLES = [2012 + 2*k for k in range(6)]

COLUMN_ALIASES = {
    "cand_id": ["FEC ID#", "FEC ID", "CANDIDATE ID", "CAND_ID"],
    "state": ["STATE ABBREVIATION", "STATE", "STATE ABBR"],
    "district": ["D", "DISTRICT", "DIST"],
    "cand_name_first": ["CANDIDATE NAME (First)", "FIRST NAME"],
    "cand_name_last": ["CANDIDATE NAME (Last)", "LAST NAME"],
    "party": ["PARTY", "PARTY ABBR"],
    "votes": ["GENERAL VOTES", "GENERAL ELECTION VOTES", "VOTES"],
    "vote_share": ["GENERAL %", "GENERAL PCT", "VOTE SHARE"],
    "outcome": ["GE WINNER INDICATOR", "WINNER", "WIN IND"]
}

def normalize(col):
    return col.strip().upper().replace("%", "PCT")
